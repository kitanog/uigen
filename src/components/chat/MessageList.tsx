                          ...
                          case "tool-invocation":
                            ...
                            const getToolLabel = () => {
                              if (tool.toolName === "str_replace_editor") {
                                const verb = args?.command === "create" ? ["Creating", "Created"]
                                  : args?.command === "delete" ? ["Deleting", "Deleted"]
                                  : ["Editing", "Edited"];
                                return { pending: `${verb[0]} ${fileName}...`, done: `${verb[1]} ${fileName}` };
                              }
                              if (tool.toolName === "file_manager") {
                                const verb = args?.command === "delete" ? ["Deleting", "Deleted"]
                                  : ["Renaming", "Renamed"];
                                return { pending: `${verb[0]} ${fileName}...`, done: `${verb[1]} ${fileName}` };
                              }
                              return { pending: "Working...", done: "Done" };
                            };
                              ...
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-neutral-50 rounded-lg text-xs border border-neutral-200">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    ...
                                    <span className="text-neutral-700">{toolLabel.done}</span>
                                  </>
                                ) : (
                                  <>
                                    <Loader2 className="w-3 h-3 animate-spin text-blue-600" />
                                    <span className="text-neutral-700">{toolLabel.pending}</span>
                                  </>
                                )}
                              </div>
                            );
